...
export function setCharTimeline(
  ...
  let screenLight: THREE.Object3D;
  ...
  character?.children.forEach((object: THREE.Object3D) => {
    if (object.name === 'Plane004') {
      object.children.forEach((child: THREE.Object3D) => {
        ...
        if (
          (child.material as THREE.MeshStandardMaterial).name === 'Material.027'
        ) {
  ...
  const neckBone = character?.getObjectByName('spine005') as THREE.Object3D;
